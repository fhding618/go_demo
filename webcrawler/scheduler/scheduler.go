...
type Scheduler interface {
	...
	// 参数poolBaaseArgs代表池基本参数的容器。
	// 参数crawDepth代表了需要被爬取的网页的最大深度值。深度大于此值的网页会被忽略。
...
func (sched *myScheduler) Start(
	...
	defer func() {
		if p := recover(); p != nil {
			errMsg := fmt.Sprintf("Fatal Schedulaer Error: %s\n", p)
			log.Println(errMsg)
	...
	if err != nil {
		errMsg :=
			fmt.Sprintf("Occur error when gen analyzer pool: %s\n", err)
		return errors.New(errMsg)
	}
...
func (sched *myScheduler) activateAnalyzers(respParsers []anlz.ParseResponse) {
	go func() {
		for {
			...
			go sched.analyzer(respParsers, resp)
		}
	}()
}

func (sched *myScheduler) analyzer(respParsers []anlz.ParseResponse, resp base.Response) {
	defer func() {
		if p := recover(); p != nil {
...
func (sched *myScheduler) openItemPipeline() {
	go func() {
		sched.itemPipeline.SetFailFase(true)
...
func (sched *myScheduler) saveReqToCache(req base.Request, code string) bool {
	httpReq := req.HttpReq()
	if httpReq == nil {
		log.Println("Ignore the request! It's HTTP request is invalid!")
		return false
	}
	reqUrl := httpReq.URL
	if reqUrl == nil {
